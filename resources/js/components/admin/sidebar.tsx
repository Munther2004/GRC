        ...
        <aside className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-60 lg:flex-col">
            <div className="flex grow flex-col overflow-y-auto bg-sidebar border-r border-sidebar-border">
                ...
                <div className="flex h-14 shrink-0 items-center gap-2.5 px-5 border-b border-sidebar-border">
                    <svg viewBox="0 0 76 65" className="h-4 w-4 fill-sidebar-foreground" aria-hidden>
                        <path d="M37.5274 0L75.0548 65H0L37.5274 0Z" />
                    </svg>
                    <span className="font-semibold text-[13px] tracking-tight text-sidebar-foreground">grc</span>
                    <span className="ml-auto font-mono text-[10px] text-sidebar-foreground/40 uppercase tracking-wider">{roleName}</span>
                </div>
                ...
                <nav className="flex flex-1 flex-col gap-2 px-2 py-4">
                    ...
                    <NavGroup 
                        ...
                        items={mainNavigation
                            ...
                            .map(item => ({ ...item, badge: getBadge((item as any).badgeKey) }))}
                        currentUrl={url}
                    />
                ...
                <div className="border-t border-sidebar-border p-3">
                    <div className="flex items-center gap-2.5 rounded-md px-2 py-1.5 hover:bg-sidebar-accent transition-colors">
                        <div className="w-7 h-7 rounded-full bg-linear-to-br from-neutral-700 to-neutral-900 flex items-center justify-center shrink-0 ring-1 ring-sidebar-border">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-sidebar-foreground/45 truncate leading-tight">{auth.user.email}</p>
                        </div>
                        <Link
                            href="/logout"
                            method="post"
...
function NavGroup({ 
    ...
    return (
        <div>
            <button
                onClick={onToggle}
                className="w-full flex items-center justify-between px-3 py-1.5 text-[10px] font-medium text-sidebar-foreground/50 uppercase tracking-[0.12em] hover:text-sidebar-foreground/70 transition-colors"
            >
                <span>{name}</span>
        ...
        <li>
            ...
            >
                <item.icon className={cn(
                    ...
                    isActive ? "text-sidebar-foreground" : "text-sidebar-foreground/45 group-hover:text-sidebar-foreground/80"
                )} />
