...
import { ScrollFocusMode, FocusScrollWrapper } from "@/components/ui/scroll-focus-mode"
import AdminLayout from "@/layouts/admin-layout"
import { Link, usePage } from "@inertiajs/react"
import type { SharedProps } from "@/types"
import { AlertTriangle, ArrowRight, ArrowUpRight, FileText, Loader2, Plus, Sparkles, Upload, XCircle, Zap } from "lucide-react"
...
type Risk = {
    ...
    impact: number; likelihood: number; risk_level: string; risk_score: number; status: string
}

type HeatmapRisk = { id: number; title: string; likelihood: number; impact: number; score: number; status: string }
...
type HealthScore = {
    health_score: number
    grade: 'A' | 'B' | 'C' | 'D' | 'F'
    components: { compliance: number; critical_risks: number; evidence_quality: number; overdue_items: number; open_risks: number }
    raw: { critical_risks: number; open_risks: number; overdue_items: number; approval_rate: number; compliance_basis: 'evidence' | 'self_assessed' }
}

type AppetiteCounts = {
    name: string
    escalated: number; review: number; acceptable: number
...
function StatTiles({ tiles }: { tiles: StatTile[] }) {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-px bg-border rounded-xl overflow-hidden border border-border">
            {tiles.map((t) => (
                <div key={t.label} className="bg-card px-5 py-4 flex flex-col justify-between min-h-[104px]">
                    <div className="flex items-center gap-1.5">
                        <span className={`w-1 h-1 rounded-full ${dot(t.tone)}`} />
                        <p className="text-[11px] font-medium text-muted-foreground tracking-tight uppercase">
                            {t.label}
                        </p>
                    </div>
                    <div className="mt-auto">
                            ...
                            <div className="mt-2 h-[3px] w-full rounded-full bg-muted overflow-hidden">
                                <div
                                    className={`h-full rounded-full ${
                                        (t.progress ?? 0) >= 70 ? 'bg-emerald-400'
                                        : (t.progress ?? 0) >= 40 ? 'bg-amber-400'
                                        : 'bg-red-400'
                                    }`}
                        ...
                        {t.hint && (
                            <p className="mt-2 text-[11px] text-muted-foreground/70">{t.hint}</p>
                        )}
                    </div>
                </div>
            ))}
...
function DashboardHero({ healthScore }: { healthScore: HealthScore }) {
    ...
    return (
        <div className="relative overflow-hidden rounded-xl border border-border bg-card">
            ...
            <div className="absolute inset-0 bg-linear-to-br from-background/0 via-background/60 to-background" aria-hidden />
            ...
            <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-6 p-6 md:p-8">
                ...
                <div className="flex items-center gap-6">
                    <div className="text-right">
                        <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Health</p>
                        <p className={`text-5xl font-semibold leading-none tabular-nums ${accent}`}>
                            {healthScore.grade}
                        </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
            {actions.map(a => (
                <Link
                    key={a.label}
                    href={a.href}
                    className="group inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground/80 hover:text-foreground hover:border-foreground/30 transition-colors"
                >
                    <a.icon className="w-3 h-3 text-muted-foreground group-hover:text-foreground transition-colors" />
                    {a.label}
                </Link>
            ))}
        </div>
        ...
        <>
            <div className="grid md:grid-cols-2 gap-3">
                <button
                    onClick={generate}
                    disabled={generating}
                    className="group relative overflow-hidden rounded-xl border border-border bg-card p-4 text-left hover:border-foreground/20 transition-colors disabled:opacity-60"
                >
                    <div className="flex items-center gap-3">
                        <div className="shrink-0 w-8 h-8 rounded-md border border-border bg-background flex items-center justify-center">
                            <Sparkles className="w-4 h-4 text-foreground/80" />
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-foreground">AI Executive Summary</p>
                            <p className="text-xs text-muted-foreground">Board-ready PDF with AI narrative</p>
                        </div>
                        {generating
                ...
                <Link
                    ...
                    className="group relative overflow-hidden rounded-xl border border-border bg-card p-4 hover:border-foreground/20 transition-colors"
                >
                    <div className="flex items-center gap-3">
                        <div className="shrink-0 w-8 h-8 rounded-md border border-border bg-background flex items-center justify-center">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-foreground">Executive Dashboard</p>
                            <p className="text-xs text-muted-foreground">Printable one-page compliance view</p>
                        </div>
                        <ArrowRight className="w-4 h-4 text-muted-foreground group-hover:text-foreground group-hover:translate-x-0.5 transition-all shrink-0" />
                    </div>
                </Link>
            ...
            {generating && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="rounded-xl border border-border bg-popover p-6 flex flex-col items-center gap-3 max-w-xs mx-4">
                        ...
                        <p className="text-xs text-muted-foreground text-center">AI is analysing your GRC data</p>
                    </div>
                </div>
            )}
            {toast && (
                <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2 rounded-md border border-red-500/30 bg-popover px-4 py-3 text-sm">
...
function HealthBreakdown({ healthScore }: { healthScore: HealthScore }) {
    const components = [
        { label: 'Compliance',        value: healthScore.components.compliance,       max: 40 },
        { label: 'Critical Risks',    value: healthScore.components.critical_risks,   max: 20 },
        { label: 'Evidence Quality',  value: healthScore.components.evidence_quality, max: 20 },
        { label: 'Overdue Items',     value: healthScore.components.overdue_items,    max: 10 },
        { label: 'Open Risks',        value: healthScore.components.open_risks,       max: 10 },
    ]

    return (
        <Card>
            <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium">Health Breakdown</CardTitle>
                        ...
                        <div key={c.label}>
                            <div className="flex items-center justify-between text-xs mb-1">
                                ...
                                <span className="font-mono tabular-nums text-muted-foreground">
                                    {c.value}<span className="opacity-40">/{c.max}</span>
                                </span>
                            </div>
...
function KriTrends({ snapshots }: { snapshots: KriSnapshot[] }) {
        ...
        return (
            <Card>
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium">KRI Trends</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="py-8 text-center">
                        <p className="text-sm text-muted-foreground">Not enough data yet</p>
                        <p className="text-xs text-muted-foreground/70 mt-1">Trends appear after the first few nightly snapshots.</p>
                    </div>
                </CardContent>
            </Card>
        )
    ...
    return (
        <div className="grid lg:grid-cols-2 gap-4">
                ...
                <CardContent>
                    <ResponsiveContainer width="100%" height={220}>
                        <LineChart data={chartData} margin={{ top: 5, right: 8, left: -16, bottom: 0 }}>
                            ...
                            <Line type="monotone" dataKey="compliance" stroke="#34d399" strokeWidth={1.5} dot={{ r: 2, fill: '#34d399' }} activeDot={{ r: 4 }} />
                        </LineChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
            <Card>
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium">Open Risks Distribution</CardTitle>
                </CardHeader>
                <CardContent>
                    <ResponsiveContainer width="100%" height={220}>
                        <BarChart data={chartData} margin={{ top: 5, right: 8, left: -16, bottom: 0 }}>
                            ...
                            <Bar dataKey="low"      name="Low"      stackId="a" fill="#22c55e" radius={[2, 2, 0, 0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
...
export default function AdminDashboard({
    stats, recentRisks, recentActivity, trendData, heatmap, kpis,
    ruleAdjustments, lastSchedulerRun, kriSnapshots, healthScore, appetiteCounts,
}: Props) {
    ...
    const tiles: StatTile[] = [
        {
            ...
            tone: kpis.risk_exposure > 50 ? 'bad' : kpis.risk_exposure >= 20 ? 'warn' : 'ok',
            progress: kpis.risk_exposure,
        },
        {
            label: 'Compliance',
            value: kpis.evidence_weighted_compliance !== null
                ? `${kpis.evidence_weighted_compliance}%`
                : `${stats.compliance_score}%`,
            hint: kpis.evidence_weighted_compliance !== null ? 'evidence-backed' : 'self-assessed',
            tone: (kpis.evidence_weighted_compliance ?? stats.compliance_score) >= 70 ? 'ok'
        ...
        },
        {
            ...
            tone: kpis.evidence_approval_rate >= 70 ? 'ok' : kpis.evidence_approval_rate >= 40 ? 'warn' : 'bad',
        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                ...
                {unreadCount > 0 && (
                    <Link
                        href="/notifications"
                        className="group flex items-center gap-3 rounded-lg border border-border bg-card hover:bg-accent/40 px-4 py-3 transition-colors"
                    >
                        <span className={`inline-flex w-1.5 h-1.5 rounded-full ${hasCritical ? 'bg-red-400' : 'bg-amber-400'} animate-pulse`} />
                        <AlertTriangle className={`w-4 h-4 shrink-0 ${hasCritical ? 'text-red-400' : 'text-amber-400'}`} />
                        <div className="flex-1 min-w-0">
                            <p className="text-sm text-foreground">
                                <span className="font-medium">{unreadCount} item{unreadCount > 1 ? 's' : ''}</span>
                ...
                {appetiteCounts && (
                    <Link href="/risk-appetite" className="group flex items-center gap-3 text-xs">
                        ...
                        <span className="ml-auto text-muted-foreground group-hover:text-foreground transition-colors flex items-center gap-1">
                ...
                <FocusScrollWrapper className="grid lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 animate-in slide-in-from-bottom-4" style={{ animationDelay: '0ms' }}>
                ...
                <div className="grid lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 animate-in slide-in-from-bottom-4" style={{ animationDelay: '300ms' }}>
                        <RiskTrendChart trendData={trendData} />
                    </div>
                    <div className="animate-in slide-in-from-bottom-4" style={{ animationDelay: '400ms' }}>
                        <RecentAlerts activity={recentActivity} />
                    </div>
                </div>

                <div className="grid lg:grid-cols-2 gap-4">
                    <div className="animate-in slide-in-from-bottom-4" style={{ animationDelay: '500ms' }}>
                        <ComplianceChart score={stats.compliance_score} />
                    </div>
                    <div className="animate-in slide-in-from-bottom-4" style={{ animationDelay: '600ms' }}>
                        <Card>
                            <CardHeader className="pb-2">
                            ...
                            <CardContent>
                                <p className="text-3xl font-semibold tabular-nums">{ruleAdjustments}</p>
                ...
                <p className="text-[11px] font-mono text-muted-foreground/50 text-center pb-2">
