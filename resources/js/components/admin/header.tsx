...
function notificationIcon(type: string) {
    ...
    switch (type) {
        ...
        case 'pending_evidence':   return <FileCheck   className={cls} style={{ color: '#C9A962' }} />
        case 'critical_risk':      return <AlertTriangle className={cls} style={{ color: '#8B2635' }} />
        case 'overdue_risk':       return <Shield      className={cls} style={{ color: '#B07840' }} />
        default:                   return <Bell        className={cls} style={{ color: '#9C8B7A' }} />
    }
}
    ...
    return (
        ...
        >
            <button className="lg:hidden p-2 -ml-2" aria-label="Open menu">
                <Menu className="h-5 w-5" style={{ color: '#9C8B7A' }} />
            </button>

            {/* Search */}
            <div className="flex flex-1 items-center">
                <div className="relative w-full max-w-sm">
                    <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2" style={{ color: '#9C8B7A' }} />
                    <input
                        type="search"
                        placeholder="Search the archives..."
                        className="h-8 w-full rounded pl-9 pr-12 text-sm outline-none transition-all duration-300 font-body"
                        style={{ background: 'rgba(37,30,25,0.8)', border: '1px solid #4A3F35', color: '#E8DFD4' }}
                    ...
                    <kbd
                        ...
                        style={{ background: '#1C1714', borderColor: '#4A3F35', color: '#9C8B7A' }}
                    >
                        ⌘K
                    </kbd>
                </div>
            ...
            <div className="relative" ref={dropdownRef}>
                ...
                >
                    ...
                    {unreadCount > 0 && (
                        <span
                            ...
                            style={{ background: '#8B2635', color: '#E8DFD4' }}
                        >
                ...
                {open && (
                    <div
                        ...
                        style={{ background: '#251E19', border: '1px solid #4A3F35', boxShadow: '0 12px 40px rgba(0,0,0,0.65)' }}
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid #4A3F35' }}>
                            <span className="font-heading text-base" style={{ color: '#E8DFD4' }}>
                                Notifications
                                {unreadCount > 0 && (
                                    <span
                        ...
                        <div className="max-h-80 overflow-y-auto">
                            {recent.length === 0 ? (
                                <div className="px-4 py-8 text-center font-body italic text-sm" style={{ color: '#9C8B7A' }}>
                                ...
                                recent.map(n => (
                                    <div
                                        ...
                                        style={{
                                            borderBottom: '1px solid rgba(74,63,53,0.4)',
                                            background: n.is_read ? 'transparent' : 'rgba(201,169,98,0.03)',
                                        }}
                                        onMouseEnter={e => (e.currentTarget.style.background = 'rgba(61,51,43,0.5)')}
                                        onMouseLeave={e => (e.currentTarget.style.background = n.is_read ? 'transparent' : 'rgba(201,169,98,0.03)')}
                                    >
                                        <button onClick={() => handleNotificationClick(n)} className="flex flex-1 gap-3 text-left min-w-0">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm truncate font-body" style={{ color: n.is_read ? '#9C8B7A' : '#E8DFD4' }}>
                                                    {n.title}
                                                </p>
                                                <p className="font-body text-xs italic line-clamp-2 mt-0.5" style={{ color: '#9C8B7A' }}>
                                                    {n.message}
                                                </p>
                        ...
                        {/* Footer */}
                        <div className="px-4 py-2.5" style={{ borderTop: '1px solid #4A3F35' }}>
                            <Link
                                href="/notifications"
