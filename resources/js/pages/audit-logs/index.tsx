        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[
                        { label: 'Total Events', value: stats.total, icon: ScrollText, color: 'text-blue-500' },
                        { label: 'Today',        value: stats.today, icon: Clock,      color: 'text-green-500' },
                        { label: 'This Week',    value: stats.week,  icon: Calendar,   color: 'text-purple-500' },
                ...
                <Card>
                    <CardContent className="p-4 space-y-3">
                        <div className="flex flex-wrap gap-3">
                            ...
                            <Select value={action} onValueChange={(v: string) => { setAction(v); applyFilters({ action: v === 'all' ? '' : v }); }}>
                            ...
                            <Select value={modelType} onValueChange={(v: string) => { setModel(v); applyFilters({ model_type: v === 'all' ? '' : v }); }}>
                        ...
                        <div className="flex flex-wrap gap-3 items-center">
                            ...
                            <Button variant="outline" onClick={() => applyFilters({ search })}>Apply</Button>
                            <Button variant="ghost" onClick={() => {
                                setSearch(''); setAction('all'); setModel('all');
                                ...
                                <tbody className="divide-y divide-border">
                                    {logs.data.length === 0 ? (
                                        ...
                                        <tr key={log.id} className="hover:bg-accent/30 transition-colors">
                                            ...
                                            <td className="px-4 py-3">
                                                <Badge variant="outline" className={`capitalize text-xs ${actionColors[log.action] ?? 'bg-gray-100 text-gray-600'}`}>
                                                    {log.action}
                                                </Badge>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className={`text-xs px-2 py-1 rounded-full font-medium ${modelColors[log.model_type] ?? 'bg-gray-100 text-gray-600'}`}>
                                                    {log.model_type}
                                                </span>
                                            </td>
